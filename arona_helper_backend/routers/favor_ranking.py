...
from arona_helper_backend.utils import FavourQueryAPI, stu_alias_convert, page_count
...
async def ranking_char(
    ...
    if len(ranking_resp.favor) == 0 and char:
        ...
        ranking_resp = await API.favor_ranking(
            char=char, page=page, num=num, reverse=reverse
        )
...
async def ranking_user(
    uid: int, char: str | None = None, page: int = 1, num: int = 10
) -> JSONResponse:
    # validate
    if (num and num < 0) or (page and page < 0):
        return JSONResponse(
